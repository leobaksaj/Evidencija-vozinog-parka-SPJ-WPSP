		...
		success:function(oData)
		{
			...
			for (var i = 0; i < oData.length; i++) 
			{
				...
				var sTr = '<tr>'+
								...
								'<td><span class="glyphicon glyphicon-pencil" onclick="GetModal(\'modals.php?modal_id=azuriraj_registraciju&vozilo_id=' +oData[i].idVozila+ '\')" aria-hidden="true"></span></td>'+
...
function AzurirajRegistraciju(VoziloID) {
    if(!$('#inptRegistracija').val())
    {
        alert('Odaberite novi datum registracije!');
    }
    $.ajax({
        url: 'action.php',
        type: 'POST',
        dataType: "html",
        data: {
            action_id: 'azuriraj_registraciju',
            idVozila: VoziloID,
            Naziv: $('#inptNaziv').val(),
            Marka: $('#inptMarka').val(),
